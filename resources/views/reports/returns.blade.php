...
<style>
    ...
    .filter-form select, .filter-form input[type="date"], .filter-form input[type="checkbox"] {
        padding: 6px;
        border-radius: 6px;
...
<div class="container">
    ...
    <div class="card">
        <div class="filter-form">
            <form method="GET" action="{{ route('admin.reports.returns') }}">
                <select name="status">
                ...
                <a href="{{ route('admin.reports.returns', array_merge(request()->query(), ['export' => 'excel'])) }}" class="btn btn-secondary">Export Excel</a>
                <a href="{{ route('admin.reports.returns', array_merge(request()->query(), ['export' => 'pdf'])) }}" class="btn btn-secondary">Export PDF</a>
            </form>
        </div>
                ...
                <tbody>
                    @forelse ($returnLogs as $log)
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-{{ $log->terlambat > 0 ? 'terlambat' : 'tidak-terlambat' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="empty-state">
                                <i class="bi bi-inbox"></i> Tidak ada data pengembalian. Silakan periksa proses pengembalian atau filter yang digunakan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="pagination">
            {{ $returnLogs->appends(request()->query())->links('vendor.pagination.custom') }}
        </div>
    </div>
</div>
@endsection